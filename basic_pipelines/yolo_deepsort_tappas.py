...
class GStreamerPoseEstimationApp(GStreamerApp):
    def __init__(self, args, user_data):
        ...
        self.re_id_dewarp_so = "/usr/lib/aarch64-linux-gnu/apps/re_id/libre_id_dewarp.so" # temporaire (voir pour modifier le setup.sh)
        self.re_id_overlay_so = "/usr/lib/aarch64-linux-gnu/apps/re_id/libre_id_overlay.so"
    ...
    def reid_pipeline(self):
        pipeline = "tee name=t2 !"
        pipeline += QUEUE("bypass_queue_reid", max_size_buffers=20, leaky="no") + "hmux2.sink_0 "
        pipeline += "t2. ! queue name=pre_reid_q leaky=no max-size-buffers=10 max-size-bytes=0 max-size-time=0 ! "
        pipeline += QUEUE("queue_src_scale2", leaky="no")
        pipeline += f"videoscale ! "
        pipeline += f"video/x-raw, format={self.network_format}, width=128, height=256 ! "
        pipeline += "videoconvert n-threads=3 ! "
        pipeline += f"hailonet hef-path={self.re_id_hef_path} vdevice-key=1 force-writable=true ! "
    ...
    def get_pipeline_string(self):
        ...
        # pipeline_string += "hailomuxer name=hmux2 "
        pipeline_string += source_element
        pipeline_string += self.yolo_pipeline()

        pipeline_string += QUEUE("queue_hmux") + " hmux.sink_1 "
        pipeline_string += "hmux. ! queue name=hailo_pre_tracker leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! \
        ...
        # pipeline_string += QUEUE("queue_hmux2", leaky="no") + " hmux2.sink_1 "

        # pipeline_string += "hmux2. ! queue name=hailo_pre_gallery leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! \
        ...
        pipeline_string += f"fpsdisplaysink video-sink=fakesink name=fakesink sync={self.sync} "
